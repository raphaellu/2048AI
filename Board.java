...
/*
 * Name: Abena Bonsu Login: cs8bwalo Date: February 4, 2016 File: Board.java Sources of Help:
 * textbook, tutors
 *
 * Describe what the program does here: This Class is used to construct a Board object to be used
 * for the simulation of the game 2048. It can create a fresh board or load an already existing
 * board. In addition this class allows the user to save their current game to a new, specified
 * file. The class also allows for the board to be rotated 90 degrees to the right or left. Baed on
 * the direction passed in by the user, this class will then move tiles existing on the board in a
 * certain direction, combining tiles of the same value. The game is considered to be over when the
 * board cannot move in any direction. /
 *
 * //------------------------------------------------------------------// // Board.java // // // //
 * Class used to represent a 2048 game board // // // // Author: W16-CSE8B-TA group // // Last
 * updated by: Carlos Mattoso // // Date: 2/12/18 //
 * //------------------------------------------------------------------//
 *
 * /** Sample Board <p/> 0 1 2 3 0 - - - - 1 - - - - 2 - - - - 3 - - - - <p/> The sample board shows
 * the index values for the columns and rows Remember that you access a 2D array by first specifying
 * the row and then the column: grid[row][column]
 */
...
public class Board implements Cloneable {
   ...
   *
   * Parameters: int boardsize which represents the size of the 2048 game board to be used. Random
   * random represents the random number which be used to specifiy where (after every move) a new
   * tile should be added to the board when playing.
   *
   * Return: void
   */
   ...
   * Name: isGameOver()
   *
   * Purpose: The purpose of this method is to check whether or not the game in play is over. The
   * game is officially over once there are no longer any valid moves that can be made in any
   * direction. If the game is over, this method will return true and print the words: "Game Over!"
   * This method will be checked before any movement is ever made.
   *
   * Parameters: none
   *
   * Return: boolean: true if the game is over, and false if the game isn't over
   *
   */
   ...
   *
   * Parameters: Direction direction which represents the direction the tiles will move (if
   * possible)
   *
   ...
   * Name: canMoveLeft()
   *
   * Purpose: The purpose of this method is to test to see if a particular tile at a particular
   * index inside of the grid can be moved to the left. This method does this by checking to see if
   * a tile to the left of the specified element is either 0, or holds the same value as the tile.
   * If either is the case, then the method returns true so that the board knows that it can be
   * moved. However, it this method itself doesnt move any tiles.
   *
   * Parameters: none
   *
   * Return: boolean: returns true if a tile can be moved left. Returns false if it can't
   *
   */
   ...
   *
   * Purpose: The purpose of this method is to test to see if a particular tile at a particular
   * index inside of the grid can be moved to the right. This method does this by checking to see if
   * a tile to the right of the specified element is either 0, or holds the same value as the tile.
   * If either is the case, then the method returns true so that the board knows that it can be
   * moved. However, it this method itself doesnt move any tiles.
   *
   * Parameters: none
   *
   * Return: boolean: returns true if a tile can be moved right. Returns false if it can't
   *
   */
   ...
   *
   * Purpose: The purpose of this method is to test to see if a particular tile at a particular
   * index inside of the grid can be moved up. This method does this by checking to see if a tile
   * above the specified element is either 0, or holds the same value as the tile. If either is the
   * case, then the method returns true so that the board knows that it can be moved. However, it
   * this method itself doesnt move any tiles.
   *
   * Parameters: none
   *
   * Return: boolean: returns true if a tile can be moved up. Returns false if it can't
   *
   */
   ...
   *
   * Purpose: The purpose of this method is to test to see if a particular tile at a particular
   * index inside of the grid can be moved down. This method does this by checking to see if a tile
   * beneath the specified element is either 0, or holds the same value as the tile. If either is
   * the case, then the method returns true so that the board knows that it can be moved. However,
   * it this method itself doesnt move any tiles.
   *
   * Parameters: none
   *
   * Return: boolean: returns true if a tile can be moved down. Returns false if it can't
   *
   */
   ...
   *
   * Purpose: To move the tiles in the board as far right as they can go, while combining tiles of
   * the same value. A tile can only be combined once per movement. Each row within the grid is
   * checked for movement and successfully moved. This method relies on the use of an ArrayList to
   * shift the tiles. It places non-zero tiles into an ArrayList, and combines adjacent tiles of the
   * same value. It then adjusts the size of the ArrayList according to how many tiles of value 0
   * were removed from the list.
   *
   * Parameters: Direction direction which represents the direction in which the tiles will be moved
   *
   * Return: void
   *
   */
   ...
   *
   * Purpose: To move the tiles in the board as far left as they can go, while combining tiles of
   * the same value. A tile can only be combined once per movement. Each row within the grid is
   * checked for movement and successfully moved. This method relies on the use of an ArrayList to
   * shift the tiles. It places non-zero tiles into an ArrayList, and combines adjacent tiles of the
   * same value. It then adjusts the size of the ArrayList according to how many tiles of value 0
   ...
   *
   * Purpose: To move the tiles in the board as far down as they can go, while combining tiles of
   * the same value. A tile can only be combined once per movement. Each row within the grid is
   * checked for movement and successfully moved. This method relies on the use of an ArrayList to
   * shift the tiles. It places non-zero tiles into an ArrayList, and combines adjacent tiles of the
   * same value. It then adjusts the size of the ArrayList according to how many tiles of value 0
   ...
   *
   * Purpose: To move the tiles in the board as far up as they can go, while combining tiles of the
   * same value. A tile can only be combined once per movement. Each row within the grid is checked
   * for movement and successfully moved. This method relies on the use of an ArrayList to shift the
   * tiles. It places non-zero tiles into an ArrayList, and combines adjacent tiles of the same
   * value. It then adjusts the size of the ArrayList according to how many tiles of value 0 were
   ...
   * Name: undo()
   *
   * Purpose: The purpose of this board is to copy the previously saved board (taken from the save()
   * method) back into the grid instance variable. When undo() is called on the board, then board
   * will revert back to this previously saved board.
   *
   * Parameters: none
   *
   * Return: void
  ...
  public String toString() {
    ...
    for (int row = 0; row < GRID_SIZE; row++) {
      for (int column = 0; column < GRID_SIZE; column++)
        outputString
            .append(grid[row][column] == 0 ? "    -" : String.format("%5d", grid[row][column]));
